...
<body class="ProducTypeHome2" onload="javascript:pageInit()">
    ...
    <div id="position">您现在的位置: <a href="/" name="linkHome">巴巴运动网</a> &gt;&gt; <em>产品查询结果</em> （${pagebean.totalrecord}个）
	</div>
    ...
    <div class="browse_right">
	     <div id="divNaviTop" class="number">
	          <div class="number_l">以下查询到<span class='number_white'>${pagebean.totalrecord}</span>条结果　每页显示<span class="number_white">${pagebean.pagesize}</span>条</div>
		      <div class="turnpage">
...
<c:forEach items="${pagebean.list}" var="entry">	
		<div class="goodslist">
          <div class="goods" style="cursor:hand;background:url(<c:forEach items="${entry.styles}" var="pic">${pic.image140FullPath}</c:forEach>) center center no-repeat"><a href="/product/view?productid=${entry.id}" target="_blank">
            <img src="/images/global/product_blank.gif" alt="${entry.name}" width="140" height="168"  border="0"/></a></div>
          <div class="goods_right">
                <h2><a href="/product/view?productid=${entry.id}" target="_blank" title="${entry.name}">${entry.name}</a></h2>
			  ...
			  <c:if test="${!empty entry.brand}"> <li>品牌：${entry.brand.name}</li></c:if>
	           ...
	           <div class="message_bottom">
			        ...
			        <div class="buy"><a href="/product/view?productid=${entry.id}"><img src='/images/sale.gif' width='84' height='24' border='0' /></a></div>
	           </div>
          </div>
