...
function settleAccounts(){
	if(validateAmount()){		
		...
		form.action=form.action+"_settleAccounts";
		form.submit();
	}
}
...
/** 修改产品数量购买数量字段 **/
function changeamount(amount,productid,styleid,oldvalue){
    		var form = document.forms["buycart"];
    		if(amount>99 || amount<=0 || amount!=parseInt(amount)){
    			alert("输入的数量不正确或太大,请检查产品数量");
    			document.getElementById("amount_"+productid+"_"+styleid).value = oldvalue;
    		}else{
    		    form.action="/shopping/cart/manage_updateAmount?productid="+productid+"&styleid="+styleid+"&amount="+amount;
    			form.submit();
    		}
    	}
    ...
    <TD><TABLE cellSpacing=0 cellPadding=0 width="96%" border=0>
      <TBODY>
        <TR>
          ...
          <TD width="14%" align="left"><a href="/shopping/cart/manage_deleteAll?directUrl=${param.directUrl}"><img style="CURSOR:hand;" alt="清空购物车" src="/images/buy/az-empty-shoppingcard.gif" border="0"></a></TD>
...
<c:forEach items="${buyCart.items}" var="item"> 
       <TR vAlign="top">
        ...
        <TD align="middle" width="73"><INPUT type="text" style="WIDTH: 30px" maxLength="3" name="amount_${item.product.id }_<c:forEach items="${item.product.styles}" var="style">${style.id }</c:forEach>"
        	 id="amount_${item.product.id }_<c:forEach items="${item.product.styles}" var="style">${style.id }</c:forEach>" value="${item.amount}"    onchange="changeamount(this.value,${item.product.id},<c:forEach items="${item.product.styles}" var="style">${style.id }</c:forEach>,${item.amount})"></TD>
        <TD align="middle" width="66"><a href="/shopping/cart/manage_delete?directUrl=${param.directUrl }&productid=${item.product.id}&styleid=<c:forEach items="${item.product.styles}" var="style">${style.id }</c:forEach>" ><img height="17" src="/images/buy/delete.gif" width="45" border="0"></a></TD>
      </TR>
      <TR vAlign="top">
